...
int psrfits_create(struct psrfits *pf)
{
    ...
    char *template_file = getenv("PSRFITS_SEARCH_TEMPLATE");
    if (template_file == NULL)
        template_file = PSRFITS_SEARCH_TEMPLATE;
    printf("Opening file '%s' in search mode.\n", pf->filename);
...
{
    int row, *status, nchan, nivals, out_nbytes;
    float ftmp;
    struct hdrinfo *hdr;
    struct subint *sub;
    ...
    fits_write_col(pf->fptr, TBYTE, 17, row, 1, out_nbytes, sub->rawdata, status);
