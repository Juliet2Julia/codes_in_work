...
#bitstream = 'px4d1k_1g_iadc_2016_Jan_07_0936.bof.gz'
#bitstream = 'px4d1k_1g_iadc_2016_Jan_12_1309.bof.gz'
#bitstream='px4d1k_iadc_2016_Jan_14_1534.bof.gz'
bitstream='px8d1k_1g_2015_May_14_2221.bof.gz'
roach_board = 'roach2'
katcp_port = 7147
...
#adc buffer
adcscope_depth = 1024
#adc output demultiplex numbers
interleave_size = 4
...
center_freq = 1420			# Center frequency of the observing band (MHz)
bandwidth = 70				# Bandwidth of the observing band (MHz), implies sampling rate is 2 * bandwidth
...
#                print('mark3')
		...
		if self.recording:
			...
			self.sync_widgets(self.PULL)
			args = [os.path.join(self.pathname, 'specrecv'), dest_ip_string, str(dest_port), basename, str(nfft), str(acclen), str(interleave_size)]
			...
			self.root.after(100, self.capture_output)
                 #       print str(self.arm.get())
			if self.arm.get() != 0:
                            
                            #should match the device name==> the first parameter
	...
	def config_fpga(self):
		...
		self.fpga.write_int('reset', 1)
                print('gbe_overrun:')
                #gbe_over=self.fpga.read_uint('fft_overrun')
		gbe_over=self.fpga.read_uint('status')
                #paket_over=self.fpga.read_int('pkt_overrun')
                print gbe_over#, paket_over
